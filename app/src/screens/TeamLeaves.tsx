...
// Balance-bearing leave types and working-day helpers (weekends excluded).
// UAE federal-government weekend = Saturday+Sunday; change to [5,6] for Fri–Sat.
...
function workingDays(a: Date, b: Date): number {
  ...
  while (+cur <= +end) { if (!WEEKEND_DAYS.has(cur.getDay())) count += 1; cur.setDate(cur.getDate() + 1); }
  return count;
}
...
export function TeamLeaves() {
  ...
  const typeOpts = [{ v: '', label: rl('كل الأنواع', 'All types') }, ...opt(['سنوية', 'تعويضية', 'طارئة', 'مرضية'])];
      ...
      <div className="page-head" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 22 }}>
        ...
        {(canManage || canEditBalances) && (
          <div className="page-head-action" style={{ flex: 'none', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {canEditBalances && (
              <button onClick={() => setBalForm(true)} title={rl('عرض وتعديل أرصدة الإجازات لكل شخص', 'View and edit leave balances')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#fff', color: '#1e4634', border: '1px solid #cdd8ce', borderRadius: 11, padding: '11px 15px', fontSize: 12.5, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer', whiteSpace: 'nowrap' }}>
  ...
  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <div><Label>{rl('نوع الإجازة', 'Leave type')}</Label><Dropdown value={f.type} options={['سنوية', 'تعويضية', 'طارئة', 'مرضية'].map((v) => ({ v, label: tr(v) }))} onChange={set('type')} opt={{ block: true, size: 'sm' }} /></div>
