...
function LeaveFormFields({ leaveId, onDone, onCancel }: { leaveId: string | null; onDone: () => void; onCancel: () => void }) {
  ...
  // People available for a leave: the balances list (added by the responsible
  // person) plus the roster and anyone already on a leave — deduped.
  const balancePeople = (data.leaveBalances || []).map((b) => b.person);
  const allPeople = [...new Set([...balancePeople, ...officeNames, ...managerNames, ...data.leaves.map((l) => l.person), f.person].filter(Boolean))];
  const personOpts = allPeople.map((n) => ({ v: n, label: tr(n) }));
    ...
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div><Label>{rl('الموظف / المدير', 'Employee / manager')}</Label>
          <Dropdown value={f.person} options={personOpts} onChange={onPersonChange} opt={{ block: true, size: 'sm', placeholder: rl('اختر الشخص', 'Pick a person') }} /></div>
